...
class dagperformancetests : public UnitTestGroup
{
    public:
        ...
        /// @detail Test timgs and performce
        virtual void RunTests(bool RunAutomaticTests, bool RunInteractiveTests)
        {
            ...
            if(RunAutomaticTests)
            {
                ...
                for(vector<Whole>::iterator Iter = Rates.begin(); Iter!=Rates.end(); ++Iter)
                {
                    ...
                    // Allow a 3% variance - incosistent achievable even on even on winxp with its crappy 3.5 millisecond timer
                    if(3>Variance)
                        { temp=Testing::Success; }
                    else
                        { temp=Testing::Warning; }
                    AddTestResult("DAGFrameScheduler::TimingVariance3%", temp); // Uses overwrite if less successful behavior
                    // Allow a .1% variance - This is very achievable with an accurate microsecond timer
                    if(0.1>Variance)
                        { temp=Testing::Success; }
                    else
                        { temp=Testing::Warning; }
                    AddTestResult("DAGFrameScheduler::TimingVariance0.1%", temp);
                }
                cout << "Average Variance: " << VarianceTotal.GetAverage() << "%" << endl;
            }else{
                AddTestResult("DAGFrameScheduler::TimingVariance3%", Testing::Skipped);
                AddTestResult("DAGFrameScheduler::TimingVariance0.1%", Testing::Skipped);
            }
