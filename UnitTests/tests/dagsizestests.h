...
/// @brief A function that has part of an algorigthm to determine when cache get slower to infer its speed.
void InferCacheSize()
{
...
class dagsizestests : public UnitTestGroup
{
    public:
        ...
        /// @return Returns a String containing "sizes"
        virtual String Name()
            { return String("dagsizes"); }

        /// @copydoc Mezzanine::Testing::UnitTestGroup::RunTests
        /// @detail This doesn't actually test anything but it is useful and usually needed around the same time as unit tests
