...
void CatchProfile::Load(const String& ProfilesDir)
{
    if( !this->Loaded ) {
        ...
        FileIStream LoadStream(Resource::CombinePathAndFileName(ProfilesDir+"/",this->ProfileName+".xml"),Mezzanine::SF_Read);
...
void CatchProfile::Save(const String& ProfilesDir)
{
    ...
    /// @todo Possibly in the future instead of constructing the datastream direct, call on the resource manager to create it(future plans)
    FileOStream SaveStream(Resource::CombinePathAndFileName(ProfilesDir+"/",ProfileName+".xml"),(Mezzanine::SF_Truncate | Mezzanine::SF_Write));
