...
\begin{DoxyCompactList}\small\item\em \hyperlink{structThis}{This} creates a data structure that can help SDL(User Input Subsystem) with Ogre(graphics subsystem). \item\end{DoxyCompactList}\item 
...
Plugins.cfg is the file that determines which graphics plugins are loaded. \hyperlink{structThis}{This} is a feature of the graphics subsystem and is generally not needed outside of engine code. \begin{DoxyReturn}{Returns}
...
\hyperlink{structThis}{This} creates a data structure that can help SDL(User Input Subsystem) with Ogre(graphics subsystem). 

\begin{DoxyInternal}{For internal use only.}
\hyperlink{structThis}{This} creates a data structure that can help SDL(User Input Subsystem) with Ogre(graphics subsystem) \hyperlink{structThis}{This} returns a named parameter list with valid settings to use Ogre rendering on a pre-\/existing SDL context \begin{DoxyWarning}{Warning}
\hyperlink{structThis}{This} is an engine internal, and shouldn't be used anywhere else. For all practical purposes is return gibberish 
\end{DoxyWarning}
\end{DoxyInternal}
...
\begin{DoxyInternal}{For internal use only.}
Settings.cfg is the file that determines how graphics settings are configured by default. \hyperlink{structThis}{This} is a feature of the graphics subsystem and is generally not needed outside of engine code. \begin{DoxyReturn}{Returns}
...
\hyperlink{structThis}{This} makes use of \hyperlink{classphys_1_1World}{World} internals to Render to the screen, So it is advised against calling this directly. Currently there is no known issue with calling this directly, but it is not thread safe and is run during the main loop at the aproppriate times. Handles the actual cross platform swapping of graphics buffers. 
\begin{DoxyParams}{Parameters}
\item[{\em TheWorld}]\hyperlink{structThis}{This} is a pointer to the \hyperlink{classphys_1_1World}{World} to be rendered. \end{DoxyParams}
