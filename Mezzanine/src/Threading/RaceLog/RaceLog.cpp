...
std::atomic<RaceLog::ThreadIndexType::InternalType> RaceLog::ThreadCount{-1};
thread_local RaceLog::ThreadIndexType RaceLog::CurrentThreadIndex;
RaceLog::ThreadLogGroupType RaceLog::ThreadLogGroup;


RaceLog::ThreadIndexType RaceLog::RegisterThread()
{
...
RaceLog::ThreadIndexType RaceLog::GetThreadIndex()
{
    return CurrentThreadIndex;
}

void RaceLog::ResetRegistry()
{
    ThreadCount = -1;
    CurrentThreadIndex = ThreadIndexType{-1};
}

void RaceLog::PrepareLogGroup(const ThreadLogGroupType::size_type& MaxThreadCount)
{
...
void RaceLog::Log(const char* Message)
{
    ThreadLogGroup[CurrentThreadIndex.AsRaw()].push_back(ThreadLogEntryType(Message));
}

const RaceLog::ThreadLogType& RaceLog::GetLog(const RaceLog::ThreadLogGroupType::size_type& Index)
{
    return GetLog(ThreadIndexType{Index});
}

const RaceLog::ThreadLogType& RaceLog::GetLog(const RaceLog::ThreadIndexType& Index)
{
    return ThreadLogGroup[Index.AsRaw()];
}

RaceLog::AggregatedLogType RaceLog::AggregateLogs()
{
    // to Assemble the log
    ...
    const RaceLog::AggregatedLogType::size_type TotalCount{
        std::accumulate(ThreadLogGroup.begin(),
                        ThreadLogGroup.end(),
                        RaceLog::AggregatedLogType::size_type{0},
                        [](RaceLog::AggregatedLogType::size_type RunningTotal, const ThreadLogType& OneThreadLog)
                            { return RunningTotal + OneThreadLog.size(); }
                       )
    };
    const RaceLog::AggregatedLogType::size_type ThreadCount{ThreadLogGroup.size()};

    RaceLog::AggregatedLogType Results;
