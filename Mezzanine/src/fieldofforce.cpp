    ...
    void FieldOfForce::ProtoDeSerializeProperties(const XML::Node& SelfRoot)
    {
        ...
        if( !PropertiesNode.Empty() ) {
            if(PropertiesNode.GetAttribute("Version").AsInt() == 1) {
                ...
                MEZZ_EXCEPTION(ExceptionBase::INVALID_VERSION_EXCEPTION,"Incompatible XML Version for " + (FieldOfForce::GetSerializableName() + "Properties" ) + ": Not Version 1.");
            }
        }else{
            MEZZ_EXCEPTION(ExceptionBase::II_IDENTITY_NOT_FOUND_EXCEPTION,FieldOfForce::GetSerializableName() + "Properties" + " was not found in the provided XML node, which was expected.");
        }
    }
