...
namespace Mezzanine
{
    ...
    {
        Entresol::GetSingletonPtr()->GetPhysicsManager()->_GetPhysicsWorldPointer()->addSoftBody(this->PhysicsSoftBody,PhysicsSettings->GetCollisionGroup(),PhysicsSettings->GetCollisionMask());
        this->AttachToGraphics();
    }

    void ActorSoft::RemoveFromWorld()
    {
        Entresol::GetSingletonPtr()->GetPhysicsManager()->_GetPhysicsWorldPointer()->removeSoftBody(this->PhysicsSoftBody);
