...
#ifndef noisemodulepower_cpp
#define noisemodulepower_cpp
...
namespace Mezzanine
{
    namespace Noise
    {
        namespace Module 
        {
            ...
            PreciseReal Power::GetValue(const PreciseReal X, const PreciseReal Y, const PreciseReal Z) const
            {
                assert( this->m_pSourceModule[0] != NULL );
                assert( this->m_pSourceModule[1] != NULL );

                return MathTools::Pow( this->m_pSourceModule[0]->GetValue(X, Y, Z),
                                       this->m_pSourceModule[1]->GetValue(X, Y, Z) );
            }
        }//Module
    }//Noise
}//Mezzanine
