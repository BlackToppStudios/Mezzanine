        ...
        void SystemAddress::ProtoDeSerialize(const XML::Node& SelfRoot)
        {
            ...
            if( !SelfRoot.Empty() && SelfRoot.Name() == SystemAddress::GetSerializableName() ) {
                if( SelfRoot.GetAttribute("Version").AsInt() == 1 ) {
                    ...
                    MEZZ_EXCEPTION(Exception::INVALID_VERSION_EXCEPTION,"Incompatible XML Version for " + SystemAddress::GetSerializableName() + ": Not Version 1.");
                }
            }else{
                MEZZ_EXCEPTION(Exception::II_IDENTITY_NOT_FOUND_EXCEPTION,SystemAddress::GetSerializableName() + " was not found in the provided XML node, which was expected.");
            }
        }
