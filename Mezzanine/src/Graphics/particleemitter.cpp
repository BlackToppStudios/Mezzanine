        ...
        void ParticleEmitter::ProtoDeSerializeCustomParameters(const XML::Node& SelfRoot)
        {
            ...
            if( !CustomParametersNode.Empty() ) {
                if(CustomParametersNode.GetAttribute("Version").AsInt() == 1) {
                    ...
                    MEZZ_EXCEPTION(ExceptionBase::INVALID_VERSION_EXCEPTION,"Incompatible XML Version for " + (ParticleEmitter::GetSerializableName() + "CustomParameters" ) + ": Not Version 1.");
                }
            }else{
                MEZZ_EXCEPTION(ExceptionBase::II_IDENTITY_NOT_FOUND_EXCEPTION,ParticleEmitter::GetSerializableName() + "CustomParameters" + " was not found in the provided XML node, which was expected.");
            }
        }
